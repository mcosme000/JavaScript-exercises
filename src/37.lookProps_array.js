...
We have a lookUpProfile function that takes name and a property (prop) as arguments.
Check the following: 
...
function lookUpProfile(name, prop) {
  ...
  for (let i = 0; i < contacts.length; i++) {
    if (contacts[i].firstName === name) {
      console.log(contacts[i][prop] ? contacts[i][prop] : "No such property");
    } else {
...
//lookUpProfile(contacts);
lookUpProfile("Kristian", "lastNamess");
